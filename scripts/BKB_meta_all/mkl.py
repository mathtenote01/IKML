...
def main(
    ...
    plt.tight_layout()
    fig.savefig(
        "mkl_distribution-init.pdf",
        bbox_inches="tight",
    )
    fig.savefig(
        "mkl_distribution-init.png",
        bbox_inches="tight",
    )
        ...
        if validate:
            ...
            with futures.ThreadPoolExecutor() as executor:
                ...
                for val_batch in val_batches:
                    ...
                    # Predict
                    y_hat = model.predict(X_val)
                    meta_valid_error += torch.norm(y_hat - y_val).detach().numpy().copy()
            # for val_batch in val_batches:
            #     evaluation_error = fast_adapt_ker(
            #         batch=val_batch,
            #         model=model,
            ...
            plt.tight_layout()
            fig.savefig(
                "mkl_distribution-step{}.pdf".format(iteration),
                bbox_inches="tight",
            )
            fig.savefig(
                "mkl_distribution-step{}.png".format(iteration),
                bbox_inches="tight",
            )
    ...
    plt.tight_layout()
    fig.savefig(
        "mkl_distribution-final.pdf",
        bbox_inches="tight",
    )
    fig.savefig(
        "mkl_distribution-final.png",
        bbox_inches="tight",
    )
