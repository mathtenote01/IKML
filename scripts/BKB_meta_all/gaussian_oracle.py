...
def main(
    ...
    # s2s, lambdas
    s2s = np.geomspace(1e1, 1e8, 5)
    lambdas = np.geomspace(1e-3, 1e2, 5)
    ...
    traindata_meta = AirQualityDataLoader(k_support, k_query, split="train", t=True)
    valdata_meta = AirQualityDataLoader(k_support, k_query, split="valid", t=True)
    testdata_meta = AirQualityDataLoader(k_support, k_query, split="test", t=True)
