...
# 'meta_lr': 0.8576472763287589, 'lam': 0.4318084048691525, 'latent_d': 35, 'hidden_dim': 17, 'num_layers': 2
    # meta_lr = 0.85765
    # lam = 0.4318
    # latent_d = 35
    # hidden_dim = 17
    ...
    # traindata_[0] = AirQualityDataLoader(k_support, k_query, split="train")
    ...
    def objective(trial):
        ...
        testdata_meta = GasSensorDataLoader(k_support, k_query, split="test", t=True)
        traindata = AirQualityDataLoader(k_support, k_query, split="train")
        valdata = AirQualityDataLoader(k_support, k_query, split="test")
        testdata = AirQualityDataLoader(k_support, k_query, split="test")
        ...
        for iteration in tqdm(range(num_iterations), desc="training"):
            ...
            def _fast_adapt_boch_valid(batch, model, D, device):
                ...
                y_hat = model.predict(X_val)
                # print("Now loading...")
                # print(torch.norm(y_hat - y_val).to('cpu').detach().numpy().copy() ** 2)
                meta_valid_error += torch.norm(y_hat - y_val).to('cpu').detach().numpy().copy() ** 2
                # print(type(meta_valid_error))
            ...
            if validate:
                ...
                with futures.ThreadPoolExecutor() as executor:
                    ...
                    for val_batch in val_batches:
                        ...
                        # y_val = y_val.to(device).float()
                        # # adapt algorithm
                        # print("the size of training dataset are {}, {}".format(X_tr.size(), y_tr.size()))
                        # print("the size of validation dataset are {}, {}".format(X_val.size(), y_val.size()))
                        # model.kernel.sample_features(D)
                        # model.fit(X_tr, y_tr)
                        # # Predict
                        # y_hat = model.predict(X_val)
